...
// creating a stopword LOCAL corpus
        ...
        List<String> word_pair_count = chats.flatMap(v -> IntStream.range(1, v._4().toString().split(" ").length).mapToObj(i -> v._4().toString().split(" ")[i-1].concat(" " + v._4().toString().split(" ")[i])).collect(Collectors.toList()).stream().iterator())
        ...
        Path out = Paths.get("out/whatsapp_chats/WhatsApp_Chat_word_count2.txt");
        Path out_csv = Paths.get("out/whatsapp_chats/WhatsApp_Chat.csv");

        List<String> chat_list = null;

       if(!Files.exists(out_csv))
          { chat_list = chats.map(v -> v.toString()).collect() ;

        Files.write(out_csv, chat_list , Charset.forName("ISO-8859-1"));}

        Files.write(out, word_pair_count , Charset.forName("ISO-8859-1"));
